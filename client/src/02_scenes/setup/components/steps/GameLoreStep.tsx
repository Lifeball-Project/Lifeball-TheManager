...
import { useSetupStore } from "06_store/useSetupStore";
import { lore } from "../../../../09_config/lore/gameLore";
...
export function GameLoreStep() {
  ...
  const handleNextLore = () => {
    if (currentLineIndex < lore.length - 1) {
  ...
  return (
    ...
    >
      <div className="bg-transparent text-black text-center p-8 w-[1000px] h-[300px] mb-12 rounded-md flex flex-col justify-start pt-2">
        ...
        <div className="mt-20">
          <p className={lore[currentLineIndex].className + " text-lg font-semibold"}>
            {lore[currentLineIndex].text}
          </p>
